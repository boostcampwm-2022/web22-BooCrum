...
export class changeRoleType1669639570300 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`boocrum\`.\`team_member\` CHANGE COLUMN \`role\` \`role\` TINYINT NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`boocrum\`.\`workspace_member\` CHANGE COLUMN \`role\` \`role\` TINYINT NOT NULL DEFAULT '0'`,
    );
  }
